...
- [x] 1.1 Add `MCPEAVESDROP_*` env var constants to `src/types/index.ts` (`MCPEAVESDROP_IPC_SOCKET`, `MCPEAVESDROP_REAL_SERVER`, `MCPEAVESDROP_SERVER_NAME`, `MCPEAVESDROP_CONFIG_PATH`, `MCPEAVESDROP_EXT_DIR`, `MCPEAVESDROP_WRAPPER_VERSION`)
...
- [x] 1.4 Create `src/mcp-wrap.ts` — `wrapEntry()` transforms a single server entry into its wrapped form; `unwrapEntry()` reconstructs original from `MCPEAVESDROP_*` env vars; `isWrapped()` detection
...
- [x] 2.1 Create `src/proxy/stdio-wrapper.ts` with version comment `// MCPEAVESDROP_WRAPPER_VERSION=1` on line 1
- [x] 2.2 Implement startup: read `MCPEAVESDROP_REAL_SERVER`, `MCPEAVESDROP_IPC_SOCKET`, `MCPEAVESDROP_EXT_DIR` from env; check extension directory existence; if missing, invoke self-heal and exec real server
- [x] 2.3 Implement self-heal: read `MCPEAVESDROP_CONFIG_PATH` and `MCPEAVESDROP_SERVER_NAME`, restore the original entry in `mcp.json`, spawn the real server with `stdio: 'inherit'`, and exit
...
- [x] 4.1 Create `src/wrapper-deploy.ts` — `deployWrapper(context)`: resolves stable path (`~/.mcpEavesdrop/stdio-wrapper.js`), reads version from deployed file if present, compares with bundled, copies if missing or stale
- [x] 4.2 Handle Windows path (`%USERPROFILE%\.mcpEavesdrop\`) and directory creation with `fs.mkdirSync({ recursive: true })`
...
- [x] 5.1 Create `src/monitoring-commands.ts` — `registerMonitoringCommands(context)` wires up `mcpEavesdrop.enableMonitoring` and `mcpEavesdrop.disableMonitoring`
...
- [x] 5.4 Implement `disableMonitoring`: read `mcp.json`, unwrap all `MCPEAVESDROP_IPC_SOCKET` entries via `unwrapEntry()`, write file, show confirmation; handle no wrapped entries gracefully
- [x] 5.5 Add `mcpEavesdrop.enableMonitoring` and `mcpEavesdrop.disableMonitoring` to `contributes.commands` in `package.json` with display titles "MCP Eavesdrop: Enable MCP Monitoring" and "MCP Eavesdrop: Disable MCP Monitoring"
...
- [x] 7.3 Implement `~/.mcpEavesdrop/` deletion with `fs.rmSync({ recursive: true, force: true })`
...
- [x] 7.6 Write unit test for `lifecycle.ts`: mock filesystem with two `mcp.json` files (VS Code and Cursor paths), both containing wrapped entries; verify both are fully restored and `~/.mcpEavesdrop/` is removed
...
- [x] 9.2 Remove workspace-to-user-config fallback: revert the `.vscode/mcp.json` lookup in `readMcpConfig()` so `mcpEavesdrop.showMcpConfig` reads only the IDE user-level `mcp.json`; workspace config support is deferred to a future phase
- [x] 9.3 Extend `showProxyConfigSnippet()` to display all server types — HTTP servers get a proxy snippet; stdio servers are listed with command/args and a note directing the user to "MCP Eavesdrop: Enable MCP Monitoring"
- [x] 9.4 Annotate output with the source config file path so the user knows which file was read
- [x] 9.5 Show a success notification toast after generating the snippet ("MCP config summary generated from <source>. See Output → MCP Eavesdrop.")
