...
### Decision 2: Copy wrapper to `~/.mcpEavesdrop/stdio-wrapper.js` (stable path)

The extension writes the compiled wrapper to `~/.mcpEavesdrop/stdio-wrapper.js` once on first enable, and overwrites it only when the embedded wrapper version changes. The `mcp.json` entry points to this stable path, not the versioned extension directory.
...
The wrapper embeds a version comment (`// MCPEAVESDROP_WRAPPER_VERSION=1`). On activate, the extension reads the deployed wrapper version and overwrites if stale.

### Decision 3: Original config embedded in env vars (self-describing entries)

Each wrapped `mcp.json` entry stores its original configuration inside `MCPEAVESDROP_REAL_SERVER` (JSON-serialized) and cleanup metadata in `MCPEAVESDROP_CONFIG_PATH`, `MCPEAVESDROP_SERVER_NAME`, `MCPEAVESDROP_EXT_DIR`, `MCPEAVESDROP_WRAPPER_VERSION`. Restoration reads these directly from the file — no separate backup store needed.

**Alternatives considered:**
- *Backup file (`mcp.json.mcpEavesdrop-backup`)*: Can't be committed; secrets may leak; stale if user edits the file after wrapping.
...
```
macOS / Linux:  Unix domain socket  → /tmp/mcpEavesdrop-extension.sock
Windows:        Named pipe          → \\.\pipe\mcpEavesdrop-extension
```

The `MCPEAVESDROP_IPC_SOCKET` env var holds the platform-appropriate address. Both the wrapper and the `vscode:uninstall` script detect `process.platform === 'win32'` at runtime.

### Decision 5: Three-layer uninstall protocol

1. **`vscode:uninstall` script** (`dist/lifecycle.js`): Runs as a plain Node.js process (no VS Code API) after extension removal. Scans all known config paths for all IDEs and platforms, restores wrapped entries, deletes `~/.mcpEavesdrop/`.
2. **Self-healing wrapper** (defense in depth): On startup, the wrapper checks whether `MCPEAVESDROP_EXT_DIR` still exists on disk. If not, it restores its own `mcp.json` entry and `exec()`s the real server, replacing itself transparently.
3. **`mcpEavesdrop.disableMonitoring` command** (user escape hatch): Available at any time for manual restore.

### Decision 6: Explicit enable/disable commands (no auto-activation)

The extension never modifies `mcp.json` automatically on activate. The user must run `mcpEavesdrop.enableMonitoring` explicitly. A Quick Pick lets them choose user-level config, workspace config (future), or both.
...
**User edits wrapped entry manually** → `MCPEAVESDROP_REAL_SERVER` becomes stale. Restoration produces the reconstructed original, not the user's modified version. Mitigation: document this clearly; the escape hatch is to disable monitoring before editing.
...
**`~/.mcpEavesdrop/` directory creation** → On first enable, the extension creates this directory. It is removed by the uninstall script. On Windows, this is `%USERPROFILE%\.mcpEavesdrop\`.

## Migration Plan

1. Extension activates → checks for existing wrapped entries (stale wrapper detection)
2. User runs `mcpEavesdrop.enableMonitoring` → IDE/platform detected, config path shown, warning shown, config rewritten
...
No data migration required — no persistent state beyond `mcp.json` and `~/.mcpEavesdrop/stdio-wrapper.js`.

### Decision 8: `mcpEavesdrop.showMcpConfig` reads IDE user-level `mcp.json` only (this phase)
