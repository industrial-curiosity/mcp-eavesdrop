...
The wrapper SHALL parse the stdio byte stream for complete MCP JSON-RPC messages and POST `tool_call_started`, `tool_call_completed`, and `tool_call_failed` events to the proxy HTTP server at the address in `MYAI_IPC_SOCKET`. Telemetry SHALL be fire-and-forget — failures MUST NOT interrupt the stdio relay.
...
- **WHEN** the POST to `MYAI_IPC_SOCKET` fails or times out
...
- **WHEN** `MYAI_IPC_SOCKET` is not connectable at wrapper startup
